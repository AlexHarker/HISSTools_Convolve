...
std::vector<unsigned int> gAudioDevNInputs;
std::vector<unsigned int> gAudioDevNOutputs;
std::vector<std::string> gMIDIInputDevNames;
std::vector<std::string> gMIDIOutputDevNames;
std::vector<std::string> gAudioIDDevNames;

unsigned int gNumInputs = 0;
unsigned int gNumOutputs = 0;
...
void ProbeAudioIO()
{
  ...
  gAudioIDDevNames.clear();
	gAudioDevNInputs.clear();
	gAudioDevNOutputs.clear();
	
  unsigned int nDevices = gDAC->getDeviceCount();

  for (int i=0; i<nDevices; i++)
  {
	// Ensure number of channels in case of error is zero....
	...
	gAudioDevNInputs.push_back(info.inputChannels);
	gAudioDevNOutputs.push_back(info.outputChannels);
                  ...
                  void *userData )
{
  ...
  if (gVecElapsed > N_VECTOR_WAIT) // wait N_VECTOR_WAIT * iovs before processing audio, to avoid clicks
  {
    for (i=0; i<nFrames; i++)
    {
      ...
      if (gBufIndex == 0)
      {
		  double *inputs[gPluginInstance->NInChannels()];
		  double *outputs[gPluginInstance->NOutChannels()];
		  
		  for (j=0; j < gNumInputs; j++)
			inputs[j] = inputBufferD + i + (j * nFrames);
		  for (; j < gPluginInstance->NInChannels(); j++)
			  inputs[j] = NULL;
		  for (j=0; j < gNumOutputs; j++)
			  outputs[j] = outputBufferD + i + (j * nFrames);
		  for (; j < gPluginInstance->NInChannels(); j++)
			  outputs[j] = NULL;
	  ...
	  for (j=0; j < gNumOutputs; j++)
		outputBufferD[i + (j * nFrames)] *= (gFadeMult * APP_MULT);
...
bool TryToChangeAudio()
{
  ...
  {
    return InitialiseAudio(inputID, outputID, samplerate, iovs, gPluginInstance->NInChannels(), gPluginInstance->NOutChannels(), gState->mAudioInChanL - 1, gState->mAudioOutChanL - 1);
  }
...
bool InitialiseAudio(unsigned int inId,
                     ...
                     unsigned int sr,
                     unsigned int iovs,
                     unsigned int inChnls,
                     unsigned int outChnls,
                    ...
                    )
{
  ...
  inChnls = inChnls > gAudioDevNInputs[inId] ? gAudioDevNInputs[inId] : inChnls;	
  outChnls = outChnls > gAudioDevNOutputs[outId] ? gAudioDevNOutputs[outId] : outChnls;
  //gPluginInstance->SetIO(inChnls, outChnls);

  gNumInputs = inChnls;
  gNumOutputs = outChnls;

  RtAudio::StreamParameters iParams, oParams;
  iParams.deviceId = inId;
  iParams.nChannels = inChnls;
  iParams.firstChannel = inChanL;

  oParams.deviceId = outId;
  oParams.nChannels = outChnls;
